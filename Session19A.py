# Explore Set
# Unique

john_followers = {"fionna", "sia", "jack", "jim", "georgia", "kia"}
jake_followers = {"anna", "sia", "kia"}
fionna_followers = {"sia", "joe"}


mutual_followers = john_followers.intersection(jake_followers)
print(mutual_followers)

result = john_followers.issubset(fionna_followers)
print(result)

A = {1, 2, 3, 4, 5}
B = {4, 5, 6 ,7, 8}

# C = A + B
# print("C is ",C)   # Not Supported

D = A - B
print("D is ",D)

E = A ^ B   # Common elements deleted
print("E is ",E)

F = A | B   # All elements added without repeating
print("F is ",F)

A.clear()
print("A is ",A)
print("Length of A is ",len(A))

# Create a Set
my_set = set()

# print(john_followers)
# print(type(john_followers))
# print(len(john_followers))

# numbers = list(range(10, 101, 10))
# numbers1 = set(numbers)
# print
# print("numbers:", numbers)
# print("numbers1:", numbers1)

# numbers1.add(101)
# numbers1.add(201)
# numbers1.add(301)

# print("numbers1 is:", numbers1)
# numbers1.pop()
# numbers1.pop()
# print("numbers1 is:", numbers1)


# numbers1.remove(90)
# numbers1.remove(30)
# numbers1.remove(101)
# print("2. numbers1 is:", numbers1)




